...
def main():
    ...
    api_key = get_api_key()
    if not api_key:
        print("Error: Please set environment variable DEEPSEEK_API_KEY", file=sys.stderr)
    ...
    if len(sys.argv) < 2:
        ...
        print("Example: python3 sh-copilot.py 'list all files in current directory'", file=sys.stderr)
        print("Example: python3 sh-copilot.py 'find processes containing python'", file=sys.stderr)
        sys.exit(1)
    ...
    if shell_command:
        ...
        print("Translation failed, please check network connection and API key", file=sys.stderr)
        sys.exit(1)
